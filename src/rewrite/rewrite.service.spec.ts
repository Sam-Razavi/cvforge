...
describe('RewriteService', () => {
  ...
  describe('enqueue', () => {
    it('creates a DB record and adds job to the queue', async () => {
      ...
      const dto: Partial<RewriteDto> = { jobDescription: 'Software Engineer role' };

      const result = await service.enqueue(dto as RewriteDto, 'cv text');

      expect(prisma.rewriteJob.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ jobDescription: 'Software Engineer role' }),
        }),
      );
      ...
      expect(result).toMatchObject({ jobId: 'q-1', recordId: 'rec-1', status: 'queued' });
    });

    it('stores apiKeyId on the record when provided', async () => {
      prisma.rewriteJob.create.mockResolvedValue({ id: 'rec-2' });

      await service.enqueue({ jobDescription: 'Dev' } as RewriteDto, 'cv', 'key-abc');

      expect(prisma.rewriteJob.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ apiKeyId: 'key-abc' }) }),
      );
    });
    ...
    it('omits apiKeyId when not provided', async () => {
      ...
      await service.enqueue({ jobDescription: 'Dev' } as RewriteDto, 'cv');
  ...
  describe('getResult', () => {
    it('throws NotFoundException when record does not exist', async () => {
      ...
      await expect(service.getResult('ghost')).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ConflictException when job is still processing', async () => {
      ...
      await expect(service.getResult('rec-1')).rejects.toThrow(ConflictException);
    });
