...
describe('OpenAIService', () => {
  ...
  describe('rewriteCv', () => {
    it('returns the model text content', async () => {
      ...
      const result = await service.rewriteCv('old cv', 'job desc', 'en', 'professional');
    ...
    it('returns empty string when content is null', async () => {
      mockCreate.mockResolvedValueOnce(makeCompletion(null as unknown as string));
  ...
  describe('generateCoverLetter', () => {
    it('returns cover letter text', async () => {
      mockCreate.mockResolvedValueOnce(makeCompletion('Dear Hiring Manager...'));

      const result = await service.generateCoverLetter('cv text', 'job desc', 'en', 'confident');
  ...
  describe('analyzeMatch', () => {
    ...
    it('requests json_object response format', async () => {
      mockCreate.mockResolvedValueOnce(makeCompletion('{"matchScore":0,"keywordsAdded":[],"missingKeywords":[]}'));
