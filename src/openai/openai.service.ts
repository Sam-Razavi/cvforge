...
import { buildMatchAnalysisPrompt, MatchAnalysis } from './prompts/match-analysis.prompt';
...
export class OpenAIService {
  ...
  async rewriteCv(
    ...
    const { system, user } = buildRewriteCvPrompt(cvText, jobDescription, language, tone);
  ...
  async generateCoverLetter(
    ...
    const { system, user } = buildCoverLetterPrompt(rewrittenCv, jobDescription, language, tone);
  ...
  async analyzeMatch(cvText: string, jobDescription: string): Promise<MatchAnalysis> {
