...
export class RewriteProcessor extends WorkerHost {
  ...
  async process(
    job: Job<RewriteJobData, RewriteJobResult>,
  ): Promise<RewriteJobResult> {
    ...
    const rewrittenCv = await this.openai.rewriteCv(
      cvText,
      jobDescription,
      language,
      tone,
    ...
    const { matchScore, keywordsAdded } = await this.openai.analyzeMatch(
      rewrittenCv,
      jobDescription,
    );
  ...
  async onFailed(job: Job<RewriteJobData> | undefined, error: Error) {
    ...
    if (job?.data?.jobRecordId) {
      await this.prisma.rewriteJob
        ...
        .catch((e) =>
          this.logger.error(`Failed to persist error state: ${e.message}`),
        );
